...
const validationRules = {
  // 订单验证规则
  createOrder: [
    body('warehouse').notEmpty().isLength({ max: 200 }).trim()
      .custom(value => !containsSQLKeywords(value)).withMessage('输入包含非法字符'),
    body('goods').notEmpty().isLength({ max: 500 }).trim()
      .custom(value => !containsSQLKeywords(value)).withMessage('输入包含非法字符'),
  ...
  updateOrder: [
    ...
    body('warehouse').optional().isLength({ max: 200 }).trim()
      .custom(value => !containsSQLKeywords(value)).withMessage('输入包含非法字符'),
    body('goods').optional().isLength({ max: 500 }).trim()
      .custom(value => !containsSQLKeywords(value)).withMessage('输入包含非法字符'),
  ...
  createQuote: [
    ...
    body('price').isFloat({ min: 0.01, max: 999999.99 }).withMessage('价格必须在0.01-999999.99之间'),
