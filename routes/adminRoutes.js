...
// 管理员登录验证规则
const adminLoginValidation = [body('password').notEmpty().isLength({ min: 4 }).trim()];
...
// 获取系统统计信息
router.get('/stats', authenticateToken, requireRole(ROLES.ADMIN), (req, res) => {
  const stats = {
...
// 获取所有订单（管理员视图）
router.get('/orders', authenticateToken, requireRole(ROLES.ADMIN), (req, res) => {
...
// 获取用户活动记录
router.get('/user-activities', authenticateToken, requireRole(ROLES.ADMIN), (req, res) => {
  const page = parseInt(req.query.page) || 1;
